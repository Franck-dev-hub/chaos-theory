...
lines = [
    ax.plot(res[:, 0], res[:, 1], res[:, 2], "-", lw=1)[0] for res in res_list
]
balls = [
    ax.plot(res[0:1, 0], res[0:1, 1], res[0:1, 2], "o", lw=1)[0]
    for res in res_list
]
...
def init():
    for line, ball, res in zip(lines, balls, res_list):
        ...
        ball.set_data([res[0, 0]], [res[0, 1]])
        ball.set_3d_properties([res[0, 2]])
...
def animate(i, res_list, lines):
    ...
    for line, res, ball in zip(lines, res_list, balls):
        j = i * SUB_SAMPLING
        line.set_data(res[:j, 0], res[:j, 1])
        line.set_3d_properties(res[:j, 2])
        ball.set_data([res[j, 0]], [res[j, 1]])  # listes
        ball.set_3d_properties([res[j, 2]])
    return lines + balls  # , time_text
